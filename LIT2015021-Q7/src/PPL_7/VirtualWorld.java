...
public class VirtualWorld {
   ...
   /** 
     ...
     */
      ...
      /**
       * Method to find the girlfriend ( if any ) of each boy in  a list of boys using one of three implementtations according to 
       * entered choice "1" unsorted array search "2" binary search on sorted array "default" hash table
       * @param ch choice
       * @throws FileNotFoundException
       * @throws IOException 
       */ 
    public void findGirlfriendName(int ch) throws FileNotFoundException, IOException{
        BufferedReader br2=new BufferedReader(new FileReader("boy_names.txt"));
        System.out.println("List of boys whose Girlfriend's name is to be found(if any)");
        SearchStructure s;
        ...
        switch(ch){
            case 1:
                System.out.println("USING UNSORTED ARRAY DATA STRUCTURE");
        
                s=new UnsortedSearch();
                
                break;
                case 2:
                    System.out.println("USING SORTED ARRAY DATA STRUCTURE");
                s=new BinarySearch();
                
                break;
                default:
                    System.out.println("USING HASH TABLE DATA STRUCTURE");
                s=new HashSearch();
                
                
        }
        for(int i=0;i<couple_ob.length;i++){
                    s.storeData(couple_ob[i].bf.name,couple_ob[i].gf.name);
                    
                }
                for(int i=0;i<arr.length;i++){
                    System.out.println();
                    String res=s.search(arr[i]);
                    if(res==null){
                        System.out.println(arr[i]+" has No Girlfriend");
                    }
                    else{
                        System.out.println(arr[i]+"'s girlfriend is "+res);
                    }
                }
