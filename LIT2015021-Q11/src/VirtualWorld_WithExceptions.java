/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
//package PPL_3;

import java.io.*;
import java.util.*;

/**
 * <h1>This class simulates the Virtual World in which there exist some girls, boys, couples and a variety of gifts.</h1>
 * <h2>It includes code segments which generate exceptions that are handled in try and catch blocks.</h2>
 * <p>Arrays of objects of type Girl, Boy,Gift and Couple are included as instance variables.
 * <p>Class responsibility is to simulate real world activities like match-making and gifting through the member functions.
 * 
 * 
 */
public class VirtualWorld_WithExceptions {
    
    Girl[] girl_ob; 
    Boy[] boy_ob;
    Gift[] gift_ob;
    Couple[] couple_ob;
    /** 
     * Constructor instantiates the instance variables(Arrays of different types of objects) with data generated by Testing Utility.
     * <h2>Includes try and catch blocks for handling exceptions of type "EndOfInputDataException". When "-1" is read from file EndOfInputDataException is thrown.</h2>
      * @throws FileNotFoundException if file does not exist
      * @throws IOException  if there is a problem reading a record from the file.
     * 
     */
    public VirtualWorld_WithExceptions ()throws FileNotFoundException, IOException{
        girl_ob=new Girl[12];
        boy_ob=new Boy[58];
        gift_ob = new Gift[150];
        couple_ob=new Couple[12];
        int i=0;
        String line="";
        BufferedReader br1=new BufferedReader(new FileReader("rand_girls.txt"));
        BufferedReader br2=new BufferedReader(new FileReader("rand_boys.txt"));
        BufferedReader br3=new BufferedReader(new FileReader("rand_gifts.txt"));
        //while((line=br1.readLine())!=null){
        try{
            while(true){
            line=br1.readLine();
            if(line.equals("-1")){
                throw new EndOfInputException();
            }
            else{
           String []arr=line.split(",");  
           if(Integer.parseInt(arr[5])==0){
               girl_ob[i]=new ChoosyGirl(arr[0],Integer.parseInt(arr[1]),Integer.parseInt(arr[2]),Integer.parseInt(arr[3]),Integer.parseInt(arr[4]));
           
           }
           
           else if(Integer.parseInt(arr[5])==1)
           {
               girl_ob[i]=new NormalGirl(arr[0],Integer.parseInt(arr[1]),Integer.parseInt(arr[2]),Integer.parseInt(arr[3]),Integer.parseInt(arr[4]));
           
           }
           else if(Integer.parseInt(arr[5])==2)
           {
               girl_ob[i]=new DesperateGirl(arr[0],Integer.parseInt(arr[1]),Integer.parseInt(arr[2]),Integer.parseInt(arr[3]),Integer.parseInt(arr[4]));
           
           }
           i++;
            }
            
                
            } 
       }
            catch(EndOfInputException e){
                    System.out.println("End Of Input from rand_girls.txt");
                    System.out.println();
                    br1.close();
                    i=0;
                   }
        
       try{
            while(true){
            line=br2.readLine();
            if(line.equals("-1")){
                throw new EndOfInputException();
            }
            else{
           String []arr=line.split(",");  
           if(Integer.parseInt(arr[5])==0){
               boy_ob[i]=new Miser(arr[0],Integer.parseInt(arr[1]),Integer.parseInt(arr[2]),Integer.parseInt(arr[3]),Integer.parseInt(arr[4]));
           
           }
           
           else if(Integer.parseInt(arr[5])==1)
           {
               boy_ob[i]=new Geek(arr[0],Integer.parseInt(arr[1]),Integer.parseInt(arr[2]),Integer.parseInt(arr[3]),Integer.parseInt(arr[4]));
           
           }
           else if(Integer.parseInt(arr[5])==2)
           {
               boy_ob[i]=new GenerousBoy(arr[0],Integer.parseInt(arr[1]),Integer.parseInt(arr[2]),Integer.parseInt(arr[3]),Integer.parseInt(arr[4]));
           
            }
           i++;
            }
            
                
            } 
       }
            catch(EndOfInputException e){
                System.out.println("End Of Input from rand_boys.txt");
                    System.out.println();
                    br2.close();
                    i=0;
                   }
       try{
            while(true){
            line=br3.readLine();
            if(line.equals("-1")){
                throw new EndOfInputException();
            }
            else{ 
                if(i<50){
           //line=br3.readLine();
           String []arr=line.split(",");
       
           gift_ob[i]=new EssentialGift(Integer.parseInt(arr[0]),Integer.parseInt(arr[1]));
            }
                else if(i>=50 && i<100){
           //line=br3.readLine();
           String []arr=line.split(",");
           gift_ob[i]=new UtilityGift(Integer.parseInt(arr[0]),Integer.parseInt(arr[1]));
        }
                
                else if(i>=100&& i<150){
           //line=br3.readLine();
           String []arr=line.split(",");
           gift_ob[i]=new LuxuryGift(Integer.parseInt(arr[0]),Integer.parseInt(arr[1]));
         }
            }
            i++;
            }
       }
       catch(EndOfInputException e){
           System.out.println("End Of Input from rand_gifts.txt");
                    System.out.println();
                    br3.close();
                    i=0;
                   }
       
         for(i=0; i<couple_ob.length; i++) {
           couple_ob[i]=new Couple();
           
       }
    }
     
    /**
     * Method to allocate boyfriends to each girl according to her criteria: 0-"most rich" 1-"most intelligent" 2-"most attractive".
     * <h2>Includes try and catch blocks for handling exceptions of type "InvalidCooupleFormedException". If in the newly formed couple, budget of boy is less than maintenance cost of girl, InvalidCooupleFormedException is thrown </h2>
     * 
     * 
     * @param log a BufferedWriter object
     * @throws java.io.IOException in case of error during input/output
     */    
    public void makeMatches(BufferedWriter log) throws IOException{
        Date date=new Date();
        int i,x=-1;
        int size=boy_ob.length;
        int intel[]=new int[size],rich[]=new int[size],attr[]=new int[size];
        for(i=0;i<boy_ob.length;i++){
           
            intel[i]=i;
            rich[i]=i;
            attr[i]=i;
        }
        int ch=1;
        for(i=0;i<girl_ob.length;i++){
            
            try{
        
           //while(true){
           if(ch==1){
            if(girl_ob[i].criteria==0){
                x=selectMostRich(girl_ob[i]);
            }
            else if(girl_ob[i].criteria==1){
                x=selectMostIntel(girl_ob[i]);
            }
            else{
                x=selectMostAttr(girl_ob[i]);
            }
           }
           else{
               if(girl_ob[i].criteria==0){
                x=selectMostRich2(girl_ob[i]);
            }
            else if(girl_ob[i].criteria==1){
                x=selectMostIntel2(girl_ob[i]);
            }
            else{
                x=selectMostAttr2(girl_ob[i]);
            }
               //System.out.println(girl_ob[i].name+" and "+boy_ob[x].name+" is the reformed valid couple");
               //System.out.println();
               ch=1;
           }
           if(boy_ob[x].budget<girl_ob[i].m_cost)
               throw new InvalidCoupleFormedException();
           else{
            couple_ob[i].getCommitted(girl_ob[i], boy_ob[x]);
            log.write(System.lineSeparator());
             log.write(date.toString()+" "+couple_ob[i].gf.name+" commited to "+couple_ob[i].bf.name+System.lineSeparator());
           }
           
          // }
            }
            catch(InvalidCoupleFormedException e){
                
                System.out.println(girl_ob[i].name+" and "+boy_ob[x].name+" is an invalid couple because boy's budget < girl's maintenance cost");
                System.out.println("The Couple wil be reformed");
                System.out.println();
                ch=2;
                i--;
            }
        }
        
    }
    /**
     * Method to display the couples formed.
     */
    public void displayMatches(){
        System.out.println("Couples Formed:"+System.lineSeparator());
        for(int i=0;i<couple_ob.length;i++){
            System.out.println(couple_ob[i].gf.name+" "+couple_ob[i].bf.name+System.lineSeparator());
        }
    }
   
    int selectMostRich2(Girl g){
        int i,max=0,max_b=0;
        for(i=0;i<boy_ob.length;i++){
            if(boy_ob[i].budget>max_b && boy_ob[i].min_attr<=g.attr && boy_ob[i].budget>=g.m_cost && boy_ob[i].gf==null){
                max_b=boy_ob[i].budget;
                max=i;
            }
        }
        
        return max;
    }
        int selectMostAttr2(Girl g){
        int i,max=0,max_a=-1;
        for(i=0;i<boy_ob.length;i++){
            if(boy_ob[i].attr>max_a && boy_ob[i].min_attr<=g.attr && boy_ob[i].budget>=g.m_cost && boy_ob[i].gf==null){
                max_a=boy_ob[i].attr;
                max=i;
            }
        }
       
        return max;    
        }
        int selectMostIntel2(Girl g){
        int i,max=0,max_i=-1;
        for(i=0;i<boy_ob.length;i++){
            if(boy_ob[i].i_level>max_i && boy_ob[i].min_attr<=g.attr && boy_ob[i].budget>=g.m_cost && boy_ob[i].gf==null){
                max_i=boy_ob[i].i_level;
                max=i;
            }
        }
       
        return max;
        }
        
    int selectMostRich(Girl g){
        int i,max=0,max_b=0;
        for(i=0;i<boy_ob.length;i++){
            if(boy_ob[i].budget>max_b && boy_ob[i].min_attr<=g.attr  && boy_ob[i].gf==null){
                max_b=boy_ob[i].budget;
                max=i;
            }
        }
        
        return max;
    }
        int selectMostAttr(Girl g){
        int i,max=0,max_a=-1;
        for(i=0;i<boy_ob.length;i++){
            if(boy_ob[i].attr>max_a && boy_ob[i].min_attr<=g.attr && boy_ob[i].gf==null){
                max_a=boy_ob[i].attr;
                max=i;
            }
        }
       
        return max;    
        }
        int selectMostIntel(Girl g){
        int i,max=0,max_i=-1;
        for(i=0;i<boy_ob.length;i++){
            if(boy_ob[i].i_level>max_i && boy_ob[i].min_attr<=g.attr  && boy_ob[i].gf==null){
                max_i=boy_ob[i].i_level;
                max=i;
            }
        }
       
        return max;
        }
        
    void sortGiftsByPrice(){
        int i,j;
        for(i=1;i<150;i++){
            for(j=0;j<150-i;j++)
                if(gift_ob[j].getPrice()>gift_ob[j+1].getPrice()){
                    Gift ob=gift_ob[j];
                    gift_ob[j]=gift_ob[j+1];
                    gift_ob[j+1]=ob;
                }
                else if(gift_ob[j].getPrice()==gift_ob[j+1].getPrice() && gift_ob[j].getVal()>gift_ob[j+1].getVal()){
                    Gift ob=gift_ob[j];
                    gift_ob[j]=gift_ob[j+1];
                    gift_ob[j+1]=ob;
                } 
        }
   
    }
    /**
     * Method that simulates gift exchanges between couples and logs them in text file "log.txt".
     * Each gift basket is set according to the type of boy: 0-"miser" 1-"geek" 2-"generous".
     * makes use of methods defined in class GiftSelectorOne.
     * miser always gives cheapest priced gift available till maintenance cost of girl is satisfied
     * geek gives gifts starting from cheapest till maintenance cost of girl is satisfied plus one luxury gift if budget allows
     * generous gives maximum cost gifts within their budget
     * @param log a BufferedWriter
     * @throws IOException  if there is an error while reading/writing file
     */
        public void performGifting(BufferedWriter log) throws IOException{
            sortGiftsByPrice();
            
            GiftSelectorOne ob1;
            
            ob1=new GiftSelectorOne();
            
            ob1.selectGifts(couple_ob,gift_ob,log);
        }
        /**
         * Method to get the happiness and compatibility of each couple.
         * <h2>Includes try and catch blocks for handling exceptions of type "K_OutOfBoundsException" and "InvalidValueException".</h2>
         * <br>If k is greater than the number of couples formed, K_OutOfBoundsException is thrown.
         * <br> If k is negative InvalidValueException is thrown.
         * calls the methods defined in class Stats.
         * @param k variable defined in problem statement
         */
       public void getCoupleStats(int k){
           try{
               if(k>12){
               throw new K_OutOfBoundsException();
           }
               if(k<0){
               throw new InvalidValueException();
           }
           }
           catch(K_OutOfBoundsException e){
               System.out.println();
               
               System.out.println("Previous value of k = "+k);
               k=couple_ob.length;
               System.out.println("New value of k = No of Couples formed = "+k);
           }
           catch(InvalidValueException e){
               System.out.println();
               System.out.println("Value of k is negative");
               System.out.println("Previous value of k = "+k);
               k=couple_ob.length;
               System.out.println("New value of k = No of Couples formed = "+k);
               System.out.println();
           }
            Stats s=new Stats();
            for(int i=0;i<couple_ob.length;i++){
                couple_ob[i].gf.calcHappiness(couple_ob[i].bf.money_spent);
                couple_ob[i].bf.calcHappiness(couple_ob[i].gf);
                couple_ob[i].calcHappiness();
                couple_ob[i].calcCompatibility();
            }
           
           s.sortCouplesByHappiness(couple_ob);
            s.printKHappiest(couple_ob,k );
            System.out.println();
            s.sortCouplesByCompatibility(couple_ob);
            s.printKCompatible(couple_ob, k);
        }
        void sortCouplesByHappiness(){
            for(int i=1;i<couple_ob.length;i++){
            for(int j=0;j<couple_ob.length-i;j++){
                if(couple_ob[j].happiness<couple_ob[j+1].happiness){
                    Couple ob=couple_ob[j];
                     
                    couple_ob[j]=couple_ob[j+1];
                    couple_ob[j+1]=ob;
                }
            }
        }
        }
        void sortCouplesByCompatibility(){
            for(int i=1;i<couple_ob.length;i++){
            for(int j=0;j<couple_ob.length-i;j++){
                if(couple_ob[j].compat<couple_ob[j+1].compat){
                    Couple ob=couple_ob[j];
                    couple_ob[j]=couple_ob[j+1];
                    couple_ob[j+1]=ob;
                }
            }
        }
        }
        /**
         * Method to perform break up of least k happy couples and form new relationships.
         * 
         * <h2>Includes try and catch blocks for handling exceptions of type "K_OutOfBoundsException" and "InvalidValueException".</h2>
         * @param k value of"k"
         * @param log BufferedWriter object
         * @throws IOException 
         */
         public void breakupLeastKHappy(int k, BufferedWriter log) throws IOException{
              try{
               if(k>12){
               throw new K_OutOfBoundsException();
           }
               if(k<0){
               throw new InvalidValueException();
           }
           }
           catch(K_OutOfBoundsException e){
               System.out.println("Previous value of k = "+k);
               k=couple_ob.length;
               System.out.println("New value of k = No of Couples formed = "+k);
           }
           catch(InvalidValueException e){
               System.out.println("Previous value of k = "+k);
               k=couple_ob.length;
               System.out.println("New value of k = No of Couples formed = "+k);
               System.out.println();
           }
        Date date=new Date();
        Stats s=new Stats();
        s.sortCouplesByHappiness(couple_ob);
        System.out.println();
        System.out.println("The "+k+" Least Happy Couples are: ");
        
        for(int i=couple_ob.length-1;i>couple_ob.length-1-k;i--){
            System.out.println(couple_ob[i].gf.name+" "+couple_ob[i].bf.name);
        }
        //PERFORMING BREAKUP N NEW MATCH
        for(int i=couple_ob.length-1;i>couple_ob.length-1-k;i--){
            System.out.println();
            System.out.println(couple_ob[i].gf.name+" and "+couple_ob[i].bf.name+" broke up");
            log.write(date.toString()+" "+couple_ob[i].gf.name+" and "+couple_ob[i].bf.name+" broke up"+System.lineSeparator());
            Boy ex_bf=couple_ob[i].bf;
            ex_bf.money_spent=0;
            for(int j=0;j<boy_ob.length;j++){
                if(boy_ob[j].min_attr<=couple_ob[i].gf.attr && boy_ob[j].budget>=couple_ob[i].gf.m_cost && boy_ob[j].gf==null){
                    couple_ob[i].getCommitted(couple_ob[i].gf, boy_ob[j]);
                    //couple_ob[i].gf.bf=boy_ob[j].name;
                    ex_bf.gf=null;
                    //couple_ob[i].bf=boy_ob[j];
                    //boy_ob[j].gf=couple_ob[i].gf.name;
                    System.out.println(couple_ob[i].gf.name+" is now committed to "+couple_ob[i].bf.name);
                    log.write(System.lineSeparator());
                    log.write(date.toString()+" "+couple_ob[i].gf.name+" is now commited to "+couple_ob[i].bf.name+System.lineSeparator());
                    break;
                
            }
            }
        }
        System.out.println();
    }
        
    }

